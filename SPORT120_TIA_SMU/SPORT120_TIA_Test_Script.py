...
def Write_Into_Parameter(reg1, reg2 ,reg3, reg4, bias):
...
def Read_Back_Parameter():
	iic_read_val = []
	for j in range(52):
		iic_read_val += [iic_read(0, 0x43, 1, j)]
...
def capture_screen_image(filename):
    ...
    inst.write("EXPort:FILEName 'E:\\SPORT120_OSC\\1125\\TIA\\%s.PNG'"%filename)      # file store desitination
...
def main():
    ...
    # reg4 = 0x50
    # bias = 1
    # Write_Into_Parameter(reg1, reg2, reg3, reg4, bias)            # write I2C register
    # Read_Back_Parameter()                                   # read I2C register
    # filename = "BRD5_CH4_%s_%s_%s_%s_TurnOFF_10G_3mV_EYE"%(hex(reg1), hex(reg2), hex(reg3), hex(reg4))
