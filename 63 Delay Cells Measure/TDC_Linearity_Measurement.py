...
'''
This script is used to measure the linearity of 63 Delay Cells TDC
@author: Wei Zhang
@date: Jan 31, 2019
...
def TOARaw_Transfer_Function(Data):
    ...
    for i in xrange(len(Data)):
        ...
        TOACntA = Data[i][68]<<2 | Data[i][67]<<1 | Data[i][66]
        TOACntB = Data[i][71]<<2 | Data[i][70]<<1 | Data[i][69]
        if Data[i][0] == 1:
    ...
    plt.savefig("TOA_Transfer_Function_1ps_nominal.png", dpi=fig_dpi, bbox_inches='tight')         # save figure
    plt.clf()
#================================================================================================#
def TOTRaw_Transfer_Function(Data):
    TOT_codeReg = []
    for i in xrange(len(Data)):
        if Data[i][72:104].count(1) > 1:
            print Data[i][72:104].index(1), "Wrong"
        else:
            TOTRaw = Data[i][72:104].index(1)
        TOTCntA = Data[i][106]<<2 | Data[i][105]<<1 | Data[i][104]
        TOTCntB = Data[i][109]<<2 | Data[i][108]<<1 | Data[i][107]
        if TOTRaw == 31:
            if Data[i][1] == 0:
    ...
    plt.savefig("TOT_Transfer_Function_1ps_nominal.png", dpi=fig_dpi, bbox_inches='tight')         # save figure
    plt.clf()
